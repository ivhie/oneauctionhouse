    ...
    public $sendingEmail;
     ...
     public function __construct(EmailTemplateController $emaitemplate, ShopifyService $shopifyService, AuctionItems $lot, Bidders $bidders,Bidders $bidder  )
     {
         ...
         public function publishedAuction(Request $request) : JsonResponse  {
            ...
            if($auction->product_url) { // run this,if this auction is push already to shopify
                    ...
                    try {
                        ...
                        if($auction->product_status == 'pending'){
                                //sending email
                        ...
                        if($auction->product_status == 'live') {

                                 //sending email
                                //send email using template 3
                                $bidder_id =  Bidders::where('acct_id',  $auction->owner_user_id)->first()->id;
                                $bidder = Bidders::find($bidder_id);
                                $response['shopify_product_url'] = 'https://'.env("SHOPIFY_SHOP_DOMAIN").'/products/'.$auction->product_url; 
       ...
       public function new($id=null){
             ...
             $page = array(
                 'menu'=>'pending',
                 'page_title'=>'Pending Auction Listing',
                 'subtitle'=>'Pending Auction Entry',
             ...
             return view('admin.pending-approvals-new')->with('page',$page);
       ...
       public function AddAuctionToLaravelDB(Request $request) {
             ...
             if( $item->save() ){ // success
                 ...
                 if(request()->submitedfrom == 'laravel'){ //submitted from laravel
                     return redirect('/pending-approvals/edit/'.$item->id)->with('added', $sucess_msg);
                 } else {
                 ...
                 if(request()->submitedfrom == 'laravel'){ //submitted from laravel
                     return redirect('/pending-approvals')->with('failed', 'New item failed to add!');
                 } else {
                     //submitted from shopify
        ...
        public function index(){
            ...
            return view('admin.pending-approvals')->with('page',$page);
        }
